...
private:
    ...
    volatile uint32_t ARR;  // Auto-reload register (gia tri max cua timer, tuong duong 100% duty cycle)
    volatile uint32_t *CCR; // Capture/Compare Register (gia tri de so sanh de xuat xung, tuong duong duty cycle hien tai)
public:
