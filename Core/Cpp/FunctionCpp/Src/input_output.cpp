...
// Hàm lọc vùng mù (deadband) và giới hạn tốc độ tối đa
float locVanToc(float val, float min_val, float max_val) {
    if (val > 0.001f) {
    ...
    return 0.0f; // Nếu nhiễu quanh 0 thì trả về 0
}
...
void nhanDuLieuPi()
{
    ...
    if (sscanf((char *)rxBuffer, "%f,%f", &vx, &w) == 2)
    {
        ...
        // vx = locVanToc(vx, 0.2f, 1.47f);
        // w = locVanToc(w, 0.5f, 1.2f);
...
void debugGuiDuLieuPi()
{
    ...
    // Thứ tự: x, y, theta_rad, vx, vy, w_rad_loc, theta_deg, w_mpu_tho
    doDaiGoiTin = snprintf((char *)txBuffer, sizeof(txBuffer),
                           "%.3f,%.3f,%.3f,%.3f,0.000,%.3f,%.1f,%.3f\n",
                           odom_x,
                           odom_y,
                           odom_theta_rad,
                           odom_vx,
