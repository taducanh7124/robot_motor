...
private:
    ...
    inline void SDA_OD() { *i2cPin_.sda.Reg_BSRR = i2cPin_.sda.pinMask; }
    // BSRR High 16 bits = Reset (Low) -> Tương đương ghi vào BRR
    inline void SDA_L() { *i2cPin_.sda.Reg_BSRR = (i2cPin_.sda.pinMask) << 16; }

    inline void SCL_OD() { *i2cPin_.scl.Reg_BSRR = i2cPin_.scl.pinMask; }
    inline void SCL_L() { *i2cPin_.scl.Reg_BSRR = (i2cPin_.scl.pinMask) << 16; }

    // Đọc IDR và Mask bit
    inline uint8_t readSDA()
    {
        return (*i2cPin_.sda.Reg_IDR & i2cPin_.sda.pinMask) ? 1 : 0;
    }
};
